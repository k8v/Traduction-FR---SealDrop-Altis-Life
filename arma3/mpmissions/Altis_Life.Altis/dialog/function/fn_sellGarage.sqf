...
if(life_cgar_inUse + (300) >= time) exitWith {closeDialog 0; hintSilent format["Vous ne pouvez vendre qu'un véhicule toute les 5 minutes ! Prochaine vente dans %1:%2",4 - floor ((time - life_cgar_inUse) / 60),59 - round (time - life_cgar_inUse - (floor ((time - life_cgar_inUse) / 60)) * 60)];};
