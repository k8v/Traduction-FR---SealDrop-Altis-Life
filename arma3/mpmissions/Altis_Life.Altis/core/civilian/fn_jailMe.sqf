...
if(_time <= 0) then { _time = time + (15 * 60); hintC "Merci de reporter à un Admin: JAIL_FALLBACK_15, time is zero!"; };
...
while {true} do
{
	if((round(_time - time)) > 0) then
	{
		...
		{
			hintSilent format["Temps Restant:\n %1\n\nCaution payable: %3\nMontant caution: $%2",_countDown,[life_bail_amount] call life_fnc_numberText];
		}
		else
		{
			hintSilent format["Temps Restant:\n %1\n",_countDown];
