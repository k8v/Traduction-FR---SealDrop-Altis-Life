...
if(life_inv_kidney >= 2) exitWith {hintSilent "Vous ne pouvez pas avoir besoin d'autant d'organes"};//if you already have 2 kidneys, then go sell them already, no stockpiling
if(!([true,"kidney",1] call life_fnc_handleInv)) exitWith {hintSilent "Vous ne pouvez pas stocker tous les organes !"};//if no room for kidney, you cannot take their kidney, duh, waste not want not
