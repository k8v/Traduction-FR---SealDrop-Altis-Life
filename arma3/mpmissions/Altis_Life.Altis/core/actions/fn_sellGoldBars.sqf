...
if(life_inv_goldbar == 0) exitWith {
	titleText["Vous n'avez pas de lingot d'or à vendre.","PLAIN"];
};
...
if([false,"goldbar",life_inv_goldbar] call life_fnc_handleInv) then {
	titleText[format["Vous avez vendu %1 lingot(s) d'or pour $%2",_val,[_price] call life_fnc_numberText],"PLAIN"];
