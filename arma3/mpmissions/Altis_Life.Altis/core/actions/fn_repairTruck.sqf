...
if((_veh isKindOf "Car") OR (_veh isKindOf "Ship") OR (_veh isKindOf "Air")) then
{
	if("ToolKit" in (items player)) then
	{
		...
		if(player != vehicle player) exitWith {titleText["Vous devez être à l'extérieur du véhicule pour réparer.","PLAIN"];_ui = "osefStatusBar" call BIS_fnc_rscLayer;_ui cutRsc["osefStatusBar","PLAIN"];};
		_veh setDamage 0;
		titleText["Vous avez réussi à réparer votre véhicule.","PLAIN"];
	};
};
