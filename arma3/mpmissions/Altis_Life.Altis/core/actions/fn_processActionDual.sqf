...
if ((_type == "uranium2") && (life_inv_puranium == 0)) exitWith {hintSilent "Vous n'avez aucun produit pour dissoudre l'uranium";};
...
if(_hasLicense) then
{
    ...
    if(player distance _vendor > 10) exitWith {hintSilent "Vous devez rester à moins de 10m durant le processus"; 5 cutText ["","PLAIN"]; life_is_processing = false;};
    ...
    else
{
    ...
    if(player distance _vendor > 10) exitWith {hintSilent "Vous devez rester à 10m durant le processus"; 5 cutText ["","PLAIN"]; life_is_processing = false;};
    if(life_cash < _cost) exitWith {hintSilent format["Vous avez besoin de $%1 pour transformer sans licence",[_cost] call life_fnc_numberText]; 5 cutText ["","PLAIN"]; life_is_processing = false;};
    if(!([false,_oldItem1,_oldVal] call life_fnc_handleInv)) exitWith {5 cutText ["","PLAIN"]; life_is_processing = false;};
    if(!([false,_oldItem2,_oldVal] call life_fnc_handleInv)) exitWith {5 cutText ["","PLAIN"]; life_is_processing = false;};
    if(!([true,_newItem,_oldVal] call life_fnc_handleInv)) exitWith {5 cutText ["","PLAIN"]; [true,_oldItem1,_oldVal] call life_fnc_handleInv; [true,_oldItem2,_oldVal] call life_fnc_handleInv; life_is_processing = false;};
