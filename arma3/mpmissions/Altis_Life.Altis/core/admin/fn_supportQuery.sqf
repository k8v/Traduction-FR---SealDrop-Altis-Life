...
if(!isNil {admin_query_ip}) exitWith {hintSilent "Vous êtes déja en train d'intérroger quelqu'un !"};
_text = _display displayCtrl 2903;
_info = lbData[2902,lbCurSel (2902)];
_info = call compile format["%1", _info];
if(isNil "_info") exitWith {_text ctrlSetText "Le joueur n'existe pas?";};
if(isNull _info) exitWith {_text ctrlSetText "Le joueur n'existe pas?";};
[[player],"TON_fnc_player_query",_info,false] spawn life_fnc_MP;
_text ctrlSetText "Intérrogation en cours......";