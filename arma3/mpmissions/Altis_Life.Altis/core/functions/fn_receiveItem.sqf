...
if(_diff != (parseNumber _val)) then
{
	...
	{
		hintSilent format["%1 vous à donnée %2 mais vous ne pouvier en prendre que %3 donc %4 lui a été rendu.",_from getVariable["realname",name _from],_val,_diff,((parseNumber _val) - _diff)];
	...
	else
{
	if(([true,_item,(parseNumber _val)] call life_fnc_handleInv)) then
	{
		...
		hintSilent format["%1 vous à donné %2 %3",_from getVariable["realname",name _from],_val,_type];
	}
		else
	{
		[[_from,_item,_val,_unit,false],"life_fnc_giveDiff",_from,false] spawn life_fnc_MP;
