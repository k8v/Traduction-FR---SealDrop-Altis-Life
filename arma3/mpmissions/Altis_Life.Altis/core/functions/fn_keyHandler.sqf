...
switch (_code) do
{
	...
	case 33:
	{
		if (vehicle player != player && (typeOf vehicle player) in["C_Offroad_01_repair_F","B_Truck_01_transport_F"]) exitWith {hintSilent "Sirène indisponible"};
	...
	case 39:
	{
		if(playerSide in [west] && vehicle player != player && !life_siren_active && ((driver vehicle player) == player)) then
		{
			...
			{
				titleText ["Warning Off","PLAIN"];
				_veh setVariable["siren2",false,true];
			}
				else
			{
				titleText ["Warning On","PLAIN"];
	...
	case 24:
	{
		...
		if !(license_civ_rebel) exitWith { hintSilent "Vous devez être un rebelle !"; };
		if(_shift && playerSide == civilian && !isNull cursorTarget && cursorTarget isKindOf "Man" && (isPlayer cursorTarget) && (side cursorTarget in [civilian,east]) && alive cursorTarget && cursorTarget distance player < 3.5 && !(cursorTarget getVariable "Escorting") && !(cursorTarget getVariable "restrained") && speed cursorTarget < 1) then
		{
			...
			{
				[] call life_fnc_restrainAction;
				hintSilent "Vous l'avez menotté !";
			} else {
				hintSilent "Vous n'avez pas l'objet requis";
			};
		};
	};
			...
			if(_veh isKindOf "House_F" && playerSide == civilian) then {
				if(_veh in life_vehicles && player distance _veh < 8) then {
					...
					if(_locked == 0) then {
						...
						systemChat "La porte a été vérrouillée";
					} else {
						_veh setVariable[format["bis_disabled_Door_%1",_door],0,true];
						_veh animate [format["door_%1_rot",_door],1];
						systemChat "La porte à été ouverte";
					};
				};
			} else {
				_locked = locked _veh;
				if(_veh in life_vehicles && player distance _veh < 8) then {
					if(_locked == 2) then {
						...
						systemChat "Vous avez dévérouillé le véhicule";
						...
						systemChat "Vous avez vérrouillé le véhicule";
	...
	case 26:
	{	
		if(vehicle player != player) exitWith {hintSilent "Vous êtes dans un véhicule!"};
		if(playerSide == west) then {
		
		if(!isNull life_spikestrip) exitWith {hintSilent "Vous avez placé une Herse."};
	...
	case 59: // F1
	{
		...
		closeDialog 0;
		hintc "Ce bouton est bloqué sur ce serveur.";
		_handled = false;
	};
