...
if(!(_house in life_vehicles)) exitWith {hintSilent "You need to be inside your house to place this."};
_containers = _house getVariable["containers",[]];
_houseCfg = [(typeOf _house)] call life_fnc_houseConfig;
if(count _houseCfg == 0) exitWith {}; //What the fuck happened?
if(count _containers >= (_houseCfg select 1)) exitWith {hintSilent "Vous n'avez plus assez de place dans votre maison pour placer ça !"};
...
if(_pos isEqualTo [0,0,0]) exitWith {hintSilent "Il n'y a plus d'espace libre dans votre maison"};
