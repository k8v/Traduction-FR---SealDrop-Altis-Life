...
if(isNull _vehicle) exitWith {hintSilent "Vous devez regarder le véhicule que vous voulez remplir."};
if(!(_vehicle isKindOF "LandVehicle") && !(_vehicle isKindOf "Air") && !(_vehicle isKindOf "Ship")) exitWith {};
if(player distance _vehicle > 7.5) exitWith {hintSilent "Vous devez être plus près du véhicule"};
...
titleText[format["Vous avez remplis ce %1",_displayName],"PLAIN"];
