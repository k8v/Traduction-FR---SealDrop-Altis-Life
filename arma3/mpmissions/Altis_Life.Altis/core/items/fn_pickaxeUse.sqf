...
//Mine check
if(_mine == "") exitWith {hintSilent "Vous n'êtes pas proche d'une mine."};
if(vehicle player != player) exitWith {hintSilent "Vous ne pouvez pas miner depuis votre voiture !";};

_diff = [_mine,_val,life_carryWeight,life_maxWeight] call life_fnc_calWeightDiff;
if(_diff == 0) exitWith {hintSilent "Votre inventaire est plein."};
...
if(([true,_mine,_diff] call life_fnc_handleInv)) then
{
	...
	titleText[format["Vous avez minez %2 %1",_itemName,_diff],"PLAIN"];
};
