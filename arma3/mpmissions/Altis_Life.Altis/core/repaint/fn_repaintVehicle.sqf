...
if(isNil {_vehicleData}) exitWith {hintSilent "Véhicule cheaté?.";_ui = "osefStatusBar" call BIS_fnc_rscLayer;_ui cutRsc["osefStatusBar","PLAIN"];};
if ((getPlayerUID player) != _vehOwner) exitWith {hintSilent "Tu n'es pas le proprio";_ui = "osefStatusBar" call BIS_fnc_rscLayer;_ui cutRsc["osefStatusBar","PLAIN"];};
		...
		if(player != vehicle player) exitWith {titleText["Tu dois sortir du véhicule pour le peindre!","PLAIN"];_ui = "osefStatusBar" call BIS_fnc_rscLayer;_ui cutRsc["osefStatusBar","PLAIN"];};
